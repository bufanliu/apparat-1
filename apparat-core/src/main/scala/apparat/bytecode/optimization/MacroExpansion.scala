...
class MacroExpansion(abcs: List[Abc]) extends SimpleLog {
  ...
  @inline def _validate(nominal: AbcNominalType, abc: Abc) = {
    if (nominal.inst.traits.length != 0) sys.error("No instance members are allowed.")
    ...
    for (t <- nominal.klass.traits) {
      t match {
        case AbcTraitMethod(_, _, method, _, _, _) => {
          ...
          if (method.returnType != voidName) sys.error("Macro must return void.")
  ...
  def validate() {
    macros.valuesIterator.foreach {
      case (n, a) => Futures.future(_validate(n, a))()
    }
  }
   ...
   *         asc2 use GetLocal(0) CallProp.. to call a macro which is not valid for Apparat
  ...
  @tailrec final def expand(bytecode: Bytecode, haveBeenModified: Boolean = false): Boolean = {
    ...
    @inline def insert(op: AbstractOp, property: AbcName, numArguments: Int) = {
      ...
      val myMacro = macroStack.head._1
      val macroName = myMacro.inst.name
      myMacro.klass.traits find (_.name == property) match {
        case Some(anyTrait) => {
          anyTrait match {
            case methodTrait: AbcTraitMethod => {
              if (numArguments != parameters.length) {
                sys.error("Expected " + numArguments + " arguments, got " + parameters.length + ".")
              }
              ...
              method.body match {
                case Some(body) => body.bytecode match {
                  case Some(matchedMacro) => {
                    ...
                    var replacement = ops.zipWithIndex.map {
                      p => p._1 match {
                        //
                        ...
                        //
                        case GetLocal(0) => {
                          // asc2 now use GetLocal(0) for static access member
                          //sys.error ("Illegal GetLocal(0).")
                          //                        GetLex(macroName)
                          Nop()
                        }
                            ...
                            case Some(someOPs) => {
                              if (someOPs.size == 1) {
                                someOPs.head match {
                                  ...
                                  case e@_ => sys.error("Unexpected " + e + ".")
                                }
                              } else {
                                val ret = byRefFingerOp(x) //Nop()
                                val newOps = someOPs.dropRight(1).map(_.opCopy()) ::: List(someOPs.last match {
                                    case GetProperty(aName) => GetProperty(aName)
                                    case x@_ => sys.error("Unexpexted " + x)
                                  })
                                byRefReplacementsMap += (ret -> newOps)
                                ret
                              }
                            }
                            case _ => sys.error("Unexpected " + nop + ".")
                          }
                          ...
                          byRefMap.get(parameters(x - 1)) match {
                            case Some(someOPs) => {
                              if (someOPs.size == 1) {
                                ...
                                val ret = byRefFingerOp(x) //Nop()
                                val newOps = someOPs.dropRight(1).map(_.opCopy()) ::: List(someOPs.last match {
                                    case GetProperty(aName) => SetProperty(aName)
                                    case x@_ => sys.error("Unexpexted " + x)
                                  })
                                byRefReplacementsMap += (ret -> newOps)
                                ret
                    ...
                    }.toList ::: List(Nop()) ::: List.tabulate(newLocals) {
                    ...
                    replacement = debugFile match {
                      case Some(aDebugFile) => oldDebugFile match {
                        case Some(anOldDebugFile) => List(anOldDebugFile.opCopy()) ::: replacement ::: List(aDebugFile.opCopy())
    ...
    for ((op, index) <- ops.zipWithIndex) op match {
      ...
      case CallPropVoid(property, numArguments) if balance > 0 && byRefBalance == 0 => {
        if (insert(op, property, numArguments)) {
          ...
          sys.error("Unexpected " + CallPropVoid(property, numArguments))
        }
      }
      ...
      case FindPropStrict(aName) if balance > 0 && aName == byRef => {
        if (byRefBalance > 0) sys.error("__byRef can't be nested")
        byRefBalance += 1
        removes = op :: removes
      }
      ...
      case x if balance > 0 => {
        bytecode.dump()
        sys.error("Unexpected operation " + x)
      }
      case _ =>
    }
