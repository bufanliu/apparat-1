...
class MemoryHelperExpansion(abcs: List[Abc]) extends SimpleLog {
  ...
  def sizeOf(s: Symbol): Int = {
    s match {
      ...
      case 'sbyte => 1
      case 'sshort => 2
      case _ => sys.error("Unknow type : " + s)
    }
  }

  def validateAndUpdateInfo() {
    for (nominal <- structures.valuesIterator) {
      if (nominal.inst.traits.length == 0) sys.error(nominal.name + " have no field member.")
      if (nominal.klass.traits.length != 0) sys.error(nominal.name + " must not have methods.")
      if (!nominal.inst.isSealed) sys.error(nominal.name + " must not be a dynamic class.")
      ...
      def getType(name: AbcName): Symbol = {
        name match {
          case qname: AbcQName => qname match {
            ...
            case _ => sys.error(qname + " must be Number, int, or uint")
          }
          case _ => sys.error(name + " must of type AbcQName")
        }
      }
      ...
      def getMetadataType(t: AbcTrait): Symbol = {
        t match {
          ...
          case _ => sys.error(t + " have to be of type AbcTraitSlot")
        }
      }
  ...
  @tailrec final def $expand(bytecode: Bytecode, haveBeenModified: Boolean = false): Boolean = {
    ...
    @tailrec def unwindAndSaveParameterStack(depth: Int, ret: AbstractOp = Nop(), saved: List[AbstractOp] = Nil): List[AbstractOp] = {
      if ((depth < 0) && parameters.nonEmpty) {
        val op = parameters.head
        parameters = parameters.tail
        val newSaved = (
    ...
    def throwError(msg: String) {
      bytecode.dump()
      optDebugFile match {
        case Some(debugFile) => sys.error(debugFile.file + ":" + lineNum + " => " + msg)
